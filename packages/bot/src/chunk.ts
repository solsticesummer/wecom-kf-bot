...
// build-index.ts calls these to turn a source document into retrieval-sized pieces.
//
 ...
 */
export function chunkFaq(markdown: string): string[] {
  const lines = markdown.split('\n');
  const chunks: string[] = [];
  let current: string[] | null = null; // null until we've seen the first `## ` heading
